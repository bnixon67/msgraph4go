...
func main() {
	...
	if len(os.Args) != 3 {
		log.Fatalf("usage: %s drive-id item-id\n", os.Args[0])
	}

	// get drive item
	permissions, err := msGraphClient.ListDriveItemPermissionsByID(os.Args[1], os.Args[2], query)
	if err != nil {
		log.Fatal(err)
	}

	fmt.Println(msgraph4go.VarToJsonString(permissions))
}
